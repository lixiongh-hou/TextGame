...
class NoticeView(context: Context, attrs: AttributeSet) : View(context, attrs) {
    ...
    private val paint2: Paint by lazy {
        Paint().also {
            it.isAntiAlias = true
            it.color = Color.parseColor("#FF6000")
            it.textSize = textSize
            it.textAlign = Paint.Align.LEFT
    ...
    override fun onDraw(canvas: Canvas) {
        ...
        if (size > 0) {
            ...
            canvas.drawText(s, 0, startIndex, 0f, nowDis, paint)
            val measureText = paint.measureText(s, 0, startIndex)
            canvas.drawText(s, startIndex, s.length, measureText, nowDis, paint2)
        }
         ...
         */
        if (size > 1) {
            val s = data[(position + 1) % size]
            canvas.drawText(s, 0, startIndex, 0f, nowDis + height, paint)
            val measureText = paint.measureText(s, 0, startIndex)
            canvas.drawText(s, startIndex, s.length, measureText, nowDis + height, paint2)
